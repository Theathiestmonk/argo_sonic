...
see the "LLM layer" section) instead of a fixed f-string — the intent being
that swapping GROQ_URL/GROQ_MODEL for a local model later is the only thing
that needs to change; no node should ever need to change because of it.
...
Requires GROQ_API_KEY always, SARVAM_API_KEY for real voice mode, and
DATABASE_URL (menu is DB-only, same as sonic_agent.py — run seed_db.py first).
"""
...
class LLMClient:
    def __init__(self, base_url: str = GROQ_URL, model: str = GROQ_MODEL, api_key: Optional[str] = None):
        self.base_url = base_url
        self.model = model
        self.api_key = api_key or GROQ_API_KEY

    def chat(self, system: str, user: str, *, json_mode: bool = False, temperature: float = 0.2,
              messages_override: Optional[list] = None) -> str:
