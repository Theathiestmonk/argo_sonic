...
a local Ollama model (qwen2.5:1.5b by default, see LLM_URL/LLM_MODEL) so the
robot works with no cloud dependency/per-call cost, one exception: the very
first line after the wake word (see n_intent_classify) is a small canned
greeting, not an LLM call — see wait_for_wake_word()/warm_up_llm() below for
why. Swapping LLM_URL/LLM_MODEL is the only thing needed to point this at a
different provider/model (e.g. OpenAI's gpt-4o-mini); no node should ever
...
Run modes (same three as sonic_agent.py, plus --test-mode):
    ...
    TABLE_NO=<n> SONIC_ACTION_HINT=<order|room_service|bill|deliver> set  ->
        one dispatched Table->Kitchen round trip, no wake word — heads
        straight to the table from wherever the robot currently is (no
        detour via Kitchen first), then returns to Kitchen once done.
...
DB-only, same as sonic_agent.py — run seed_db.py first).
"""
...
# Local Ollama (qwen2.5:1.5b) by default now — re-benchmarked 2026-09-12 on
# this actual Jetson against the real nav planner (ntfields_planner_node.py,
# device='cuda') running concurrently: predict_path() replanning latency was
# within ~2% of its solo baseline while Ollama served requests, and Ollama's
# own request latency only rose ~10% (1.39s->1.53s mean) under concurrent
# planner load. The GPU/memory-contention risk the earlier version of this
# comment warned about didn't materialize at this model size — a 1.5B q4
# model and NTFields' small per-goal gradient-descent solve both fit
# comfortably in the Orin's compute/memory budget without starving each
# other. Point LLM_URL/LLM_MODEL/LLM_API_KEY at OpenAI (or any OpenAI-
# compatible endpoint) instead if that ever changes — no node needs to.
...
# How long Ollama keeps qwen2.5:1.5b resident on the GPU after the last
# request before unloading it (Ollama's own idle-unload timer, "keep_alive"
# on every request — see LLMClient.chat()). Long enough to cover one full
# order-taking conversation without a mid-visit reload, short enough that
# the GPU goes back to being nav-only between customers rather than the
# model sitting loaded 24/7. Empty/unset sends no keep_alive field at all —
# harmless default against a non-Ollama LLM_URL (e.g. OpenAI), which would
# otherwise see an unrecognized field on every request.
...
# from the mic) but also more room noise. record_utterance() uses this SAME
# threshold for both "has speech started" and "has speech ended" (RMS must
# drop below it for TRAILING_SILENCE_MS straight), so if ambient noise alone
# reads above it, the room never reads as silence and a listen attempt hangs
# indefinitely (confirmed root cause of "agent gets stuck after saying hello
# and never responds" on 2026-09-11 — SILENCE_GIVEUP_TOTAL_S being inf/never
# means that hang has no upper bound either).
#
# Re-calibrated 2026-09-11 against this exact USB PnP adapter with a live
# sd.InputStream probe: ambient noise floor measured 600-860 RMS (previous
# 500 default was BELOW that — it was triggering on room noise alone, not
# real speech), while actual speech through the same mic measured
# 950-3100+ RMS. 1000 sits with real margin above the ambient ceiling and
# below normal speech onset. If listen attempts start hanging again or
# mishearing quiet room noise as speech, re-run that live probe on the
# actual deployed hardware (ambient noise floors drift — this itself was a
# regression from an earlier ~500 calibration that held with ~2 units of
# margin until room/hardware conditions changed) and adjust accordingly.
...
def menu_context_for_llm() -> str:
    """The real menu, grouped by category, fed into every extraction call
    so the LLM resolves what a guest ordered against actual item names
    (see menu_match in EXTRACTION_SCHEMA) instead of the code guessing via
    string similarity. Built fresh each call rather than cached — MENU_ITEMS
    only changes once at startup today, but this is cheap enough (33 items)
    that caching isn't worth the staleness risk if that ever changes."""
    if not MENU_ITEMS:
...
def _strip_variant_suffix(name: str) -> str:
    """"Latte (Hot)" -> "latte" — difflib's cutoff compares against the
    FULL name including any parenthetical variant, and that suffix eats
    enough of the ratio that short base names ("latte", "croissant")
    routinely fail to fuzzy-match their own menu entry (e.g. "latte" vs
    "latte (hot)" scores 0.625, below the 0.72 cutoff below). Matching the
    base name separately fixes that regardless of cutoff tuning."""
    return re.sub(r'\s*\([^)]*\)\s*$', '', name).strip().lower()


def find_menu_variants(spoken_name: Optional[str]) -> list:
    """Menu items whose base name (variant suffix stripped) matches
    spoken_name exactly — e.g. "latte" -> [Latte (Hot), Latte (Cold)].
    Used by find_menu_item() when there's exactly one such item (safe to
    auto-resolve), and by n_extract_items to ask which variant when there
    are several (genuine ambiguity — the variants can even be priced
    differently, so guessing one silently would be a billing mistake)."""
    if not spoken_name:
        return []
    key = spoken_name.strip().lower()
    seen_ids, out = set(), []
    for item in MENU_LOOKUP.values():
        if _strip_variant_suffix(item["name"]) == key and item["item_id"] not in seen_ids:
...
def find_menu_item(spoken_name: Optional[str]) -> Optional[dict]:
    """Exact-name/alias lookup, plus unambiguous base-name resolution (see
    find_menu_variants()) — deliberately NOT fuzzy (no difflib/similarity
    scoring here, not even for this legacy fallback used by menu Q&A and
    order editing). n_extract_items no longer relies on this at all for
    resolving what a guest ordered — the LLM extraction call is given the
    real menu (see menu_context_for_llm()) and resolves item names itself
    with actual menu/category context, which handles "muffin" (ambiguous —
    several flavors) or "blueberry cupcake" (doesn't exist, but the LLM can
    reasonably suggest the closest real item) far better than any string-
    similarity cutoff could — see menu_match in EXTRACTION_SCHEMA."""
    if not spoken_name:
        return None
    key = spoken_name.strip().lower()
    ...
    # Base-name match, e.g. "croissant" -> "Croissant (Plain)". Only
    # auto-resolve when there's exactly one variant — two or more (e.g.
    # "latte" -> Hot/Cold) is real ambiguity for n_extract_items to ask
    # about via find_menu_variants(), not something to guess here.
    variants = find_menu_variants(key)
    if len(variants) == 1:
        return variants[0]
    return None
    ...
    with sd.InputStream(samplerate=SAMPLE_RATE, channels=1, dtype="int16",
        ...
        while True:
            ...
            if score is not None and score > WAKE_WORD_THRESHOLD:
                warm_up_llm()  # earliest possible point — see warm_up_llm()
                return
...
# Spoken instantly on wake-word detection (see n_intent_classify) instead of
# an LLM call — same very-short-acknowledgment intent the old
# "ask_how_can_i_help" render() purpose hint asked for (see git history),
# just hardcoded so nothing blocks on warm_up_llm() below finishing before
# the robot can respond to its own name. Every other line the robot says
# still goes through render()/render_stream() as normal.
WAKE_GREETINGS = ["Hi!", "Hello!", "Yes?", "I'm here!", "I'm awake!"]


def warm_up_llm() -> None:
    """Fire-and-forget: called the instant wait_for_wake_word() detects the
    wake word, so Ollama starts loading qwen2.5:1.5b onto the GPU right
    away, in the background — see LLM_KEEP_ALIVE. Nothing in the
    conversation actually waits on this thread; the wake-word greeting
    itself is a canned WAKE_GREETINGS pick (instant, no LLM call), so by
    the time the guest's first real utterance comes back through STT for
    interpret() to process, the model has had that whole
    greeting-plus-listening window to finish loading instead of eating a
    cold-start delay on the critical path. A slow/failed warm-up isn't
    fatal — interpret()'s own LLM call just pays the cold-load cost itself
...
RENDER_PURPOSE_HINTS = {
    ...
    "ask_variant": "They asked for \"{item_name}\" — that could mean a couple of different real menu items. Ask which one they'd like — options: {options}.",
    "ask_category_alternatives": "They asked for \"{item_name}\", which isn't on the menu. Apologize briefly and warmly, then naturally mention what IS actually available in that same part of the menu — options: {options} — like a waiter listing real choices, not a robotic recitation, and ask if any of those sound good.",
...
EXTRACTION_SCHEMA = """{
  ...
  "items": [{"item_name": "<dish name AS THE GUEST SAID IT>", "menu_match": "<the EXACT item name from the menu list below that this refers to, copied character-for-character, or null if nothing on the menu is a reasonable match>", "quantity": <integer or null>, "preference": "<free text or null>"}],
...
EXPECTS_GUIDANCE = {
    ...
    "items": "They're telling you what dish(es) they want. Extract every distinct dish mentioned into \"items\", each with its own quantity/preference/menu_match. Resolve menu_match yourself from the actual menu below — match on meaning, not exact wording (typos, informal names, missing words, mis-transcriptions from speech all still count as a match). Only set menu_match=null when nothing on the menu is a reasonable interpretation of what they said (n_extract_items handles suggesting real alternatives from there — see guess_item_category()).",
    ...
    "yes_no_or_item": "This is an \"anything else?\" question. If they name a new dish, populate items[] (with menu_match resolved the same way as the items stage) — that counts as answering yes. Otherwise map their reply to yes_no/wants_more.",
...
The real menu (use this — and ONLY this — to resolve menu_match; never invent an item that isn't listed):
...
- menu_match must be copied EXACTLY (character-for-character, including capitalization and any "(Hot)"/\
"(Cold)"/etc. suffix) from the menu list above, or null — never a paraphrase, never an item not on the list. \
If the guest's phrase reasonably names exactly one real item (even mis-transcribed, informal, or missing a \
word), match it. If it's genuinely ambiguous between multiple real items (e.g. several muffin flavors and \
they just said "a muffin"), or matches nothing on the menu at all, use null rather than picking one for them.
...
def extract_slots(transcript: str, state: OrderState, expects: str) -> dict:
    ...
    for it in (result.get("items") or []):
        if isinstance(it, dict) and it.get("item_name"):
            menu_match = it.get("menu_match") or None
            if menu_match and menu_match.strip().lower() not in MENU_LOOKUP:
                # The LLM was told to copy an exact name off the real menu
                # and didn't — don't trust a name that isn't actually
                # orderable; treat it the same as "no match" rather than
                # risk n_extract_items silently keying off a hallucinated
                # item.
                print(f"[warn] LLM menu_match {menu_match!r} isn't a real menu item — treating as no match")
                menu_match = None
            items.append({
...
def n_intent_classify(state: OrderState) -> dict:
    ...
    if state.pending_seed is not None:
        ...
        prompt = random.choice(WAKE_GREETINGS)
        answer = interrupt({"prompt": prompt})
        if answer == TIMEOUT_SENTINEL:
            say("idle_timeout_farewell", state)
            state.went_idle = True
...
def n_extract_items(state: OrderState) -> dict:
    ...
    # The LLM already resolved this against the real menu (see menu_match
    # in EXTRACTION_SCHEMA / menu_context_for_llm()) — a plain dict lookup
    # on its answer, not a string-similarity guess. extract_slots() already
    # verified menu_match (if set) is a real key, so this always hits.
    match = MENU_LOOKUP.get((first.get("menu_match") or "").strip().lower()) or None
    if not match:
        # Two ways this resolves to real, concrete options instead of a flat
        # "couldn't find X" — checked most-precise first:
        #   1. Same-dish variants (e.g. "latte" -> Latte (Hot)/Latte (Cold))
        #      — a deterministic same-base-name check, not an LLM guess.
        #   2. Otherwise, a scoped category guess (guess_item_category() —
        #      a coarse, much easier judgment than naming specific dishes,
        #      asked as its own small call rather than bundled into the
        #      main extraction) resolved to EVERY real, available dish in
        #      that category via menu_items_by_category() — real menu
        #      data, not LLM recall.
        variants = find_menu_variants(first.get("item_name"))
        suggestion_kind = "variant"
        if not variants:
            category = guess_item_category(first.get("item_name"))
            if category:
                variants = menu_items_by_category(category)
                suggestion_kind = "category"
        if variants:
            if rest:
                state.pending_items.extend(rest)
            state.active_item = {
                "item_id": None, "name": first.get("item_name"), "category": None,
...
def guess_item_category(item_name: Optional[str]) -> Optional[str]:
    """Used only by n_extract_items when a dish didn't match the menu at
    all and no same-dish variant exists either — a small, single-purpose
    LLM call to pick which ONE real category it'd most plausibly belong to
    (e.g. "a muffin" -> bakery), so the guest can be offered every real
    dish actually in that category (menu_items_by_category()) instead of a
    flat "not found". Deliberately its own isolated call rather than one
    more field on extract_slots()'s already-large multi-field extraction:
    testing showed asking for a category guess inline there was unreliable
    (qwen2.5:1.5b got it right well under half the time, alongside
    genuinely wrong guesses like "cookies" for a muffin) — the exact same
    small model, asked ONLY this one question in isolation, got it right
    consistently. A model this size reasons better about one narrow
    judgment at a time than several bundled into one call."""
    cats = menu_categories()
    if not cats or not item_name:
        return None
    system = (
        "You are the understanding module for \"Sonic\", a restaurant robot. A guest asked for a dish that "
        "isn't on the menu. Which ONE of these real menu categories would it most likely belong to if the "
        "menu carried it: " + ", ".join(cats) + "? "
        "Return ONLY a JSON object: {\"category\": \"<one of the categories above, exactly, or null if none fit>\"}"
    )
    try:
        raw = llm.chat(system, item_name, json_mode=True, temperature=0.0, max_tokens=30)
        category = json.loads(raw).get("category")
    except Exception as e:
        print(f"[warn] category-guess LLM call failed: {e}")
        return None
    if category and category.strip().lower() in cats:
        return category.strip().lower()
    return None


_VARIANT_STOPWORDS = {"a", "an", "the", "of", "with", "and", "or"}


def _variant_name_words(name: str) -> set:
    return {w for w in re.sub(r'[()]', ' ', name).lower().split() if w not in _VARIANT_STOPWORDS}


def _match_variant_by_keyword(reply: str, candidates: list) -> Optional[dict]:
    """Match a guest's short reply ("hot", "I'll take the cold one",
    "blueberry") to one candidate by its DISTINCTIVE words — whatever's in
    its name that ISN'T shared by every candidate (e.g. "Latte (Hot)" vs
    "Latte (Cold)" -> "hot"/"cold"; "Blueberry Muffin" vs "Chocolate
    Muffin" -> "blueberry"/"chocolate", "muffin" dropped since it's common
    to both). A plain keyword check, tried first since it's free and the
    common case (a guest naming the flavor/qualifier directly) is exactly
    what it's built for — _match_variant_by_llm below covers what this
    misses."""
    reply_l = reply.strip().lower()
    per_candidate_words = [_variant_name_words(c["name"]) for c in candidates]
    common = set.intersection(*per_candidate_words) if per_candidate_words else set()
    scored = []
    for c, words in zip(candidates, per_candidate_words):
        hits = sum(1 for w in (words - common) if w in reply_l)
        if hits:
    ...
    if len(scored) > 1 and scored[0][0] == scored[1][0]:
        return None  # tie — genuinely ambiguous, let the LLM fallback reason about it
    return scored[0][1]


def _match_variant_by_llm(reply: str, candidates: list) -> Optional[dict]:
    """Fallback for when the guest's reply doesn't contain any candidate's
    distinctive word verbatim (e.g. "I'll go with the fruity one" for
    Blueberry vs Chocolate Muffin) — a small scoped LLM call rather than
    the full item-extraction pipeline (interpret()'s own schema/menu
    context), since all that's needed here is picking one of a handful of
    already-known real names. Worth the extra round trip now that the LLM
    is a local, effectively-free call rather than a billed cloud one."""
    names = [c["name"] for c in candidates]
    system = (
        "You are the understanding module for \"Sonic\", a restaurant robot. The guest was just asked to "
        "choose between exactly these menu options: " + "; ".join(f'\"{n}\"' for n in names) + ". "
        "Read their reply and return ONLY a JSON object: "
        "{\"choice\": \"<the EXACT option above they picked, copied character-for-character, or null if "
        "their reply doesn't clearly pick one of these options>\"}"
    )
    try:
        raw = llm.chat(system, reply, json_mode=True, temperature=0.0, max_tokens=60)
        choice = json.loads(raw).get("choice")
    except Exception as e:
        print(f"[warn] variant-choice LLM fallback failed: {e}")
        return None
    if not choice:
        return None
    choice_l = choice.strip().lower()
    for c in candidates:
        if c["name"].strip().lower() == choice_l:
            return c
    return None


def resolve_variant_choice(reply: str, candidates: list) -> Optional[dict]:
    return _match_variant_by_keyword(reply, candidates) or _match_variant_by_llm(reply, candidates)


def n_ask_variant(state: OrderState) -> dict:
    """Which of the real candidates did they mean — either same-dish
    variants (Hot vs Cold latte) or, when the guest named something not on
    the menu at all, every real dish in the category the LLM guessed it'd
    belong to (see n_extract_items' suggestion_kind). Resolved via
    resolve_variant_choice() (keyword match, then a small scoped LLM
    fallback), not routed through get_reply_or_route()/interpret(): that
    runs the full LLM item-extraction pipeline, which would likely misread
    a one-word "hot" as a (failed) dish name and trip the off-topic-
    redirect path instead of just answering the question that was
    actually asked."""
    trace_node("n_ask_variant", state)
    candidates = state.active_item.get("variant_options") or []
    purpose = "ask_variant" if state.active_item.get("suggestion_kind") == "variant" else "ask_category_alternatives"
    prompt = render(purpose, state, item_name=state.active_item["name"],
    ...
    if answer == TIMEOUT_SENTINEL:
        ...
        state.next_node = "n_respond"
        return asdict(state)
    state.conversation_history.append({"role": "user", "text": answer})
    db_log_turn(state, "user", answer)

    chosen = resolve_variant_choice(answer, candidates)
    if chosen is None:
        state.next_node = "n_ask_variant"
        return asdict(state)

    qty = state.active_item.get("quantity")
    pref = state.active_item.get("preference")
    state.active_item = {
        "item_id": chosen["item_id"], "name": chosen["name"], "category": chosen["category"],
        "quantity": qty, "preference": pref, "preference_asked": bool(pref),
    }
    state.next_node = route_active_item(state)
    return asdict(state)
...
def main_loop() -> None:
    ...
    if TEXT_MODE:
        ...
        # Goes straight from wherever the robot currently is to the
        # requested table (n_navigate_to_table, via the graph below) — no
        # detour through Kitchen first. Previously drove to Kitchen here
        # before the graph's own table trip even started, doubling every
        # dispatch into a Kitchen->Table leg for no operational reason (the
        # robot doesn't need to stage anything at Kitchen to take an order
        # or announce a delivery — see n_deliver_announce, food is already
        # loaded before dispatch). n_return_to_kitchen still sends it back
        # to Kitchen once the visit ends, unchanged.
        run_graph_session(app)
    else:
        require_api_keys()
