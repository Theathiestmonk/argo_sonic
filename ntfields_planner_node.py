...
class NTFieldsPlannerNode(LifecycleNode):
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        try:
            ...
            # Unguarded before this, an exception here propagated up through
            # the lifecycle configure transition and rclpy's own executor
            # logging only ever surfaced the bare exception message (e.g.
            # "sequence size exceeds remaining buffer") with no traceback —
            # impossible to tell which line actually threw it, or whether
            # it's even the same failure a standalone repro of this exact
            # call produces. Log the full traceback explicitly so a future
            # failure here is diagnosable from GET /nav_log alone.
            self.get_logger().error(
                f'[NTFieldsPlanner] model load failed:\n{traceback.format_exc()}')
            return TransitionCallbackReturn.FAILURE
