...
class Scan(Node):
            ...
            if self.pose_frame is None:
                ...
                if frame != self.map_frame:
                    self.get_logger().warn(
                        f'no {self.map_frame} -> {self.base_frame}; using '
                        f'{frame}. The azimuth is still recoverable — it only '
                        f'needs relative motion — but odometry drift will '
                        f'blunt the minimum.')
    ...
    def run_scan(self):
        ...
        if np.isfinite(sc).any():
            ...
            print(f'\n{100*share:.0f}% of scanned azimuths score within 5% of '
                  f'the best.')
            if share > 0.25:
                print('  The scan did NOT separate them — drive further and '
                      'revisit more places\n  before trusting this. '
                      'Translation is what makes a wrong azimuth visible.')
            else:
                print('  Clear minimum — the drive separated the azimuths.')
