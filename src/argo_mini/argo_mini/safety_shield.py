...
Two independent sensor sources, each with its own stale timeout:
  ...
  Ultrasonic sensors (Range � 4, published by serial_bridge from ESP32)
...
# ?? Depth camera ??????????????????????????????????????????????????????????????
DEPTH_STOP_DIST  = 0.30    # m   ? forward obstacle threshold
DEPTH_WIDTH_HALF = 0.20    # m   ? half-width of danger corridor (~robot width)
DEPTH_HEIGHT_MIN = -1.30   # opt Y ? ignore above robot head
DEPTH_HEIGHT_MAX =  0.05   # opt Y ? ignore floor returns
DEPTH_MIN_PTS    = 5       # minimum cloud points to confirm obstacle (noise filter)
DEPTH_STALE_SECS = 0.5     # s   ? clear camera vote if no frame arrives

# ?? Ultrasonic ????????????????????????????????????????????????????????????????
US_FRONT_DIST    = 0.30    # m   ? block forward if FL or FR reads below this
US_REAR_DIST     = 0.25    # m   ? block reverse if BL or BR reads below this
US_STALE_SECS    = 0.5     # s   ? clear US vote if no Range message arrives
...
class SafetyShield(Node):
    ...
    def _on_cloud(self, msg: PointCloud2):
        ...
        # Fast numpy path ? avoids per-point Python overhead
    ...
    def _on_us(self, msg: Range, sensor: str):
        ...
        with self._lock:
            ...
            d     = dict(self._us_dists)   # snapshot for logging outside lock
    ...
    def _on_cmd(self, msg: Twist):
        ...
        if fwd_blocked  and lin > 0.0:
            lin = 0.0   # obstacle ahead  ? kill forward, reverse still works
        if rear_blocked and lin < 0.0:
            lin = 0.0   # obstacle behind ? kill reverse, forward still works

        out = Twist()
        out.linear.x  = lin
        out.angular.z = msg.angular.z   # rotation always passes through
